...
async function checkApplications() {
  try {
    ...
    applications.forEach((app, idx) => {
      ...
      // details 필드 확인 (JSON)
      console.log(`📋 Details (원본 타입):`, typeof app.details)
      ...
      if (app.details) {
        ...
        console.log(`🌐 녹음 언어: ${(details as any)?.recordingLanguage || 'NULL'}`)
        console.log(`📂 평가 구분: ${(details as any)?.category || 'NULL'}`)
      } else {
    ...
    todayApplications.forEach(app => {
      const details = typeof app.details === 'string' ? JSON.parse(app.details) : app.details
      console.log(`  - ${app.user?.name} (${app.user?.employeeId}): ${app.slot}차수 - ${(details as any)?.recordingLanguage || 'NULL'}`)
    })
