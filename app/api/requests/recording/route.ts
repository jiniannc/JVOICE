...
/**
 * 녹음 신청 저장 API
 * 요청 바디: { date: string, slot: number, language: 'korean-english'|'japanese'|'chinese', employee: { id, name, dept? }, note? }
 */
export async function POST(req: NextRequest) {
  try {
    const body = await req.json()
    const { date, slot, language, employee, note } = body || {}
    if (!date || !slot || !language || !employee?.id || !employee?.name) {
      return NextResponse.json({ success: false, error: "Invalid payload" }, { status: 400 })
    }
    const apiKey = getEnvValue("NEXT_PUBLIC_GOOGLE_API_KEY")
    const sheetId = getEnvValue("NEXT_PUBLIC_REC_APP_SHEET_ID")
    const title = `${date}`
    // 시트 존재 확인 후 없으면 생성
    ...
    // Append values
    const headers = ["사번", "이름", "부서", "언어", "슬롯", "비고", "신청시각", "상태"]
    await ensureHeaderRow(sheetId, title, headers, apiKey)
    const row = [employee.id, employee.name, employee.dept || "", language, String(slot), note || "", new Date().toISOString(), "ACTIVE"]
